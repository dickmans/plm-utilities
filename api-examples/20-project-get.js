// GET PROJECT TAB DETAILS
// Before using this script, create a new Forge app at forge.autodesk.com (this service is for free)
// Please provide clientId and clientSecret of your Forge app in file settings.js before 
// Author: Sven Dickmans, Autodesk

/* --------------------------------------------------------------------------------------------------------
    Change Log
    - 2022-02-16: Integration in TS F3M Extensions Package
    - 2021-09-13: Moved login to utils.js
    - 2021-03-17: Initial version
   -------------------------------------------------------------------------------------------------------- */


//Options
let wsId        = '86';     // Equals to workspace Projects in default tenant
let dmsId       = '8839';


const axios     = require('axios');
const f3m       = require('../node_modules_adsk/f3m.js');
const utils     = require('../node_modules_adsk/utils.js');
const settings  = require('../settings.js');


utils.printStart([ ['Workspace', wsId], ['DMSID', dmsId] ]);

f3m.login().then(function() {
    getProjectActivities(function() {
        utils.printEnd();
    });
});


function getProjectActivities(callback) {

    let url = 'https://' + settings.tenant + '.autodeskplm360.net/api/v3/workspaces/' + wsId + '/items/' + dmsId + '/views/16';
    
    axios.get(url).then(function (response) {
        
        console.log('    Found ' + response.data.projectItems.length + ' activities on 1st level');

        console.log();
        console.log('    Available Properties');

        utils.printLine();

        for (var key in response.data.projectItems[0]) {
            console.log('    ' + key);
        }
        
        console.log();
        console.log();
        console.log('    List of activities');
        
        utils.printLine();
        
        for(projectItem of response.data.projectItems) {
            
            console.log('    ' + projectItem.startDate + ' - ' + projectItem.endDate + ' | ' + projectItem.title);
            
            if(projectItem.hasOwnProperty('projectItems')) {
            
               for(subItem of projectItem.projectItems) {
                    console.log('    ' + subItem.startDate + ' - ' + subItem.endDate + ' | > ' + subItem.title);
                }
               
            }
            
        }
        
        callback();
        
    }).catch(function (error) {
        console.log(error.message);
    });
    
}