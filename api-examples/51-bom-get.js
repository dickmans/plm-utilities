// GET BOM USING FIRST BOM VIEW DEFINITION
...
function getBOM(viewId, callback) {
    ...
    let url = 'https://' + settings.tenant + '.autodeskplm360.net/api/v3/workspaces/' + wsId + '/items/' + dmsId + '/bom?depth=10&effectiveDate=2020-06-25&revisionBias=release&rootId=' + dmsId + '&viewDefId=' + viewId;
    ...
    axios.get(url, {
        ...
        console.log();
        console.log('    ' + response.data.edges.length + ' BOM rows found');
        
        utils.printLine();
        
        let components = response.data.edges;
        
        console.log(response.data);
        
        components.sort(function(a, b){return a.itemNumber - b.itemNumber});
        ...
        for(component of components) {
            ...
            for(node of response.data.nodes) {
                if(node.item.urn === component.child) {
                    descriptor = node.item.title;
                    break;
                }
            }
            
            console.log('    - (' + component.itemNumber + ') ' + descriptor);
        }
